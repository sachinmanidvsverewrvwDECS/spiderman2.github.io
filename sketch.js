...
function setup(){
  ...
  //  sandman.addImage(sandImg)
    ...
    h4=createSprite(1203,593,30,30)
    h4.addImage(logoImg)
    h4.scale=0.1
  ...
  wallu=createSprite(302,rope.y+310,60,10)
  wallu.setCollider("rectangle",0,0,60,40)
  wallu.debug=false
  wallu.visible=false

  walla=createSprite(142,rope.y+410,60,10)
  walla.setCollider("rectangle",0,0,60,40)
  walla.debug=false
  walla.visible=false
  wallb=createSprite(1202,rope.y+480,60,10)
  wallb.setCollider("rectangle",0,0,60,40)
  wallb.debug=false
  wallb.visible=false
  ...
  bigwall=createSprite(3988,400,20,800)
...
function draw(){
    ...
    if(spider.isTouching(h4)){
        h4.destroy()
    }

    if(spider.isTouching(h5)){
        h5.destroy()
    }
    if(spider.isTouching(h6)){
        h6.destroy()
    }

    spider.collide(wallu)
    
    spider.collide(walla)
    spider.collide(wallb)
    camera.x=spider.x
...
spider.collide(wall23)
    ...
    if(spider.x>3788){
        text("Mission Complete",3688,200)
    }
    drawSprites();
...
function web(){
    if(frameCount%10===0){
        ...
        car.x=spider.x-50
        car.y=spider.y
        car.setCollider("rectangle",0,0,4,4)
        car.debug=false
